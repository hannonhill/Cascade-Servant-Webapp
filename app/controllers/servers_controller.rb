...
class ServersController < ApplicationController
  ...
  def index
    ...
    # sort by name    
    @servers.sort! { |a,b| a.values['name'] <=> b.values['name'] }
  ...
  def start
   server = (@@pwd + params[:id])
   return if not valid_server? server
   if not isrunning?(server)
    command = "#{server}/bin/startup.sh # > /dev/null 2>&1"
    forkandrun command
   end
   render :text => 'Server should be starting'
  ...
  server = (@@pwd + params[:id])
   ...
   #render :action => 'index'
   render :text => 'Server should be stopping'
   #redirect_to :action => :index
  end
  # GET /servers/1
  # GET /servers/1.xml
...
def dump(server)
  ...
  system "mysqldump -u root  -f --max-allowed-packet=512m --host=mysql #{db2_name} > /home/titus/latest#{server}dump.sql"
end
...
config = ServerConfig.new server
  ...
  system "mysql -u root --default-character-set=utf8  --host=mysql #{db2_name} < /home/titus/latestDefaultDump.sql"
...
def getAllServers
    servers = Array.new    
    Dir.entries(@@pwd).each { |child|
        if child != "." && child != ".." && FileTest.directory?(@@pwd + child) then
            if valid_server? (@@pwd + child)
                server = (@@pwd + child)
                config = ServerConfig.new server
                servers << config
            end
...
class ServerConfig
  ...
  def initialize(server)
    @server = server
    @values = Hash.new
    @values["location"] = server
    @values["name"] = server.split('/').last
